const filmes = [
  ...
  const filmesNovos = filmes.filter(filme => filme.ano > 2000);
console.log(filmesNovos);
  
  // 2) Transforme a lista de objetos da questão 1 em JSON (texto).
  const filmesNovosJSON = JSON.stringify(filmesNovos);
console.log(filmesNovosJSON);
...
console.log(filmes);
  ...
  const filmeComLista = filmes.find(filme => filme.titulo.includes("Lista"));
  console.log(filmeComLista);
  ...
  const titulosOrdenados = titulosFilmes.sort();
